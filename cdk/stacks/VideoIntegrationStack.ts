...
import { USER_STACK_NAME, VIDEO_INTEGRATION_STACK_NAME } from './stackName.ts'
...
export class VideoIntegrationStack extends Stack {
	public constructor(parent: App) {
		...
		const authRole = Role.fromRoleArn(
			this,
			'authRole',
			Fn.importValue(`${USER_STACK_NAME}:authenticatedUserRoleArn`),
		)

		const unauthRole = Role.fromRoleArn(
			this,
			'unauthRole',
			Fn.importValue(`${USER_STACK_NAME}:unauthenticatedUserRoleArn`),
		)
